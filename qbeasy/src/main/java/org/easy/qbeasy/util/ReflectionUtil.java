...
public class ReflectionUtil {
       ...
       boolean done = false;
		...
		if (object != null) {
			...
			try {
				...
				System.out.println("Não foi possível alterar o valor da propriedade via getter. Tentando a alteração direta do atributo: " 
	...
	public static void setValue(Object object, String propertyName, Object value)  {
		...
		try {
			...
			System.out.println("Não foi possível alterar o valor da propriedade via setter. Tentando a alteração direta do atributo: " 
						+ object.getClass().getSimpleName() + "." + propertyName);
		...
		for (Class<?> clazz = type; clazz != Object.class; clazz = clazz.getSuperclass()) {
			try {
				...
				break;
                            ...
                            System.out.println("O método " + methodName + " não existe na classe " + clazz.getName());
			}
		}
	...
	private static Field getClassField(Class<?> type, String propertyName) {
		...
		for (Class<?> clazz = type; clazz != Object.class; clazz = clazz.getSuperclass()) {
			try {
				...
				System.out.println("O atributo " + propertyName + "não foi encontrado na classe " + clazz.getName());
			}
		}
	...
	public static List<Field> getFields(Class<?> classe, Class<? extends Annotation> annotation) {
		...
		for (Class clazz = classe; clazz != Object.class; clazz = clazz.getSuperclass()) {
			try {
				...
				System.out.println("A anotação " + annotation.getName() + " não foi encontrada na classe " + clazz.getName());
			}
		}
